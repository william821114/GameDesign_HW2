...
public class PlayerController : MonoBehaviour {
	...
	public FireThrowerManager fireThrowerManager;
	...
	void Start () {
		...
		footstepsSound = this.GetComponent<AudioSource> ();
	}
	...
	void Update () 
	{
		...
		if (Input.GetMouseButtonDown (0) && !isGun) {
			fireThrowerManager.TryToTurnOn();
		}
		if (Input.GetMouseButtonUp (0) && !isGun) {
				fireThrowerManager.TryToTurnOff ();
		}
		...
		if (Input.GetKey (KeyCode.Q)) {
			...
			fireThrowerManager.gameObject.SetActive (false);
		} else if (Input.GetKey (KeyCode.E)) {
			isGun = false;
			gunManager.gameObject.SetActive(false);
			fireThrowerManager.gameObject.SetActive(true);
		}
		...
		if (isWalking && !footstepsSound.isPlaying) {
			if (isRightStep) {
				footstepsSound.clip = rightStepSound;
				footstepsSound.Play ();
				isRightStep = false;
			} else {
				footstepsSound.clip = leftStepSound;
				footstepsSound.Play ();
				isRightStep = true;
			}
		}
